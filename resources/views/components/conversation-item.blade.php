...
<button
    ...
    style="{{ $selected ? 'border-inline-start: 4px solid rgb(var(--primary-600)); padding-inline-start: 12px;' : 'border-inline-start: 4px solid transparent;' }}"
>
    {{-- Avatar with unread dot --}}
    <div class="relative shrink-0">
        <x-filament-chat::avatar :name="$displayName" :url="$avatarUrl" class="h-10 w-10" />
        @if ($unreadCount > 0)
            <span class="absolute -bottom-0.5 -start-0.5 h-2.5 w-2.5 rounded-full border-2 border-white dark:border-gray-900" style="background-color: rgb(var(--primary-500));"></span>
        @endif
    </div>
